#!/usr/bin/env python3
"""
System Identification for Left Arm with Friction
...
Usage:
    python sysID_left_arm_with_friction.py <json_file> [--urdf <urdf_path>] [--cutoff_vel <freq>] [--cutoff_acc <freq>]
    python sysID_left_arm_with_friction.py ../json/motion_left_arm.json
    python sysID_left_arm_with_friction.py ../json/motion_left_arm.json --t_start 10.0 --t_end 30.0
"""
...
from regressors.left_arm_regressor_wrapper import LeftArmRegressor
...
def build_regressor_matrix(model, data, q, dq_filt, ddq_filt, tau, g):
    ...
    for s in range(n_samples):
        ...
        Phi = LeftArmRegressor().compute_regressor(q_sample, dq_sample, ddq_sample, DEFAULT_GRAVITY)
...
def main():
    parser = argparse.ArgumentParser(description='System Identification for Left Arm with Friction')
    parser.add_argument('json_file', type=str, help='Path to JSON file with robot data')
    parser.add_argument('--urdf', type=str, default=None,
                       help='Path to URDF file (default: ../sysid/matlab/vr_m2_left_arm.urdf)')
    ...
    if os.path.exists(left_arm_urdf_path):
        ...
        updated_urdf_path = left_arm_urdf_path
    else:
